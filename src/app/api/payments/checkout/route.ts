...
export async function POST(request: Request) {
  ...
  if (pendingPayment) await prisma.payment.update({ where: { id: pendingPayment.id }, data: { status: "CANCELLED" } });
  const payment = await prisma.payment.create({ data: { payerId: session.user.id, jobId: job.id, amountCents, platformFeeCents, professionalAmountCents, currency: "EUR", type: "CLIENT_PAYMENT", provider: "stripe", status: "PENDING" } });
  try {
